...
var unwanted = ["http://www.nintendo.com/games/detail/qFpqVnA8gCkDnd6Qp1dm7eG9XcaHsyX8", // Puyo Puyo Tetris Digital
                "http://www.nintendo.com/games/detail/AVpEu_TNE632FoKohJdB_9tjw4IEH4j8", // BotW Master Collection
                "http://www.nintendo.com/games/detail/pXK8vQt6ojp7e9wQszsl5zKq3GgL8yCZ"  // BotW Special Edition
                ] // List of Games that should NOT be in the table
var startingCountOfUnwantedGames = unwanted.length;
var total = 0;
var wrote = 0;
var today = new Date();
var baseURL = "/home/matmoo29/moosecreates.com/Nintendo-Switch-Controller-Options-Database/";
var currentDataFile = "data.json";
var resultsDataFile = "data.json";
var reportEmailFile = "report.txt";
var changelogFile = "change.txt";
...
try{
    fs.write(baseURL + reportEmailFile, "---------------------------------------------------------\r\n" + formatDate(today) + "\r\n" , 'a');
    fs.write(baseURL + changelogFile, "<p>\n<strong>\n" + formatDate(today) + "\n</strong>\n<br>\n" , 'a');
} catch(e) {
    console.log(e);
}
...
var response = fs.read(baseURL + currentDataFile); 
if (response){
    var games = JSON.parse(response);
    allGamesFromCurrentData = games.data;
}
var unreleasedGames = [];

function convertDate(sortString){ // Change DD/MM/YYYY to MM/DD/YYYY
    return sortString.slice(3,6) + sortString.slice(0,3) + sortString.slice(6,12);
}

//Only Check Games that are Upcoming Releases (All other data should be up-to-date)
for (i = 0; i < allGamesFromCurrentData.length; i++){
    var gameDate = new Date(convertDate(allGamesFromCurrentData[i].SortDate));
    if (gameDate > today){
        unreleasedGames.push(allGamesFromCurrentData[i]);
    }
    else{
        unwanted.push(allGamesFromCurrentData[i].URL);
    }
}

logger("Succesfully opened " + allGamesFromCurrentData.length + " games from the file " + currentDataFile + ".");
logger("Removed " + (unwanted.length - startingCountOfUnwantedGames).toString(10) + " games from that list.");
logger("The resulting list of games to update is " + unreleasedGames.length + " games long.");

// This Allows for Console Logging When on a Page //
...
function get_urls(){
    page.open("https://www.nintendo.com/games/game-guide?pv=true#filter/switch|-|-|-|-|-|-|-|-|-|-|-|-|-|featured|des|-|-", function(){
...
function handle_page(url){
    var results = page.open(url.URL, function(){
        ...
        var gameData = page.evaluate(function() {
            ...
            console.log("Fetching data for " + document.title);
            var obj={};
            obj["Verified"] = false;
    ...
    window.setTimeout(next_page, 60000);
}; // End handle_page();
...
function next_page(){
    ...
    if(!url){
        logger("All Data Finished Parsing");
        if (total == wrote){
            logger("Finished Successfully! Saving Updates to JSON!");
            saveResults();
        }
        else{
            logger("Uh oh! There were only "+ wrote + " games written to the file out of " + total + ". Not saving data.");
            var errors = {};
            errors["data"] = allGamesFromCurrentData;
            fs.write(baseURL + "UnsavedData.txt" + formatDate(today), JSON.stringify(errors), 'w');
        }
        fs.write(baseURL + changelogFile, "</p>\n", 'a'); // Used to Close the ChangeLog File
